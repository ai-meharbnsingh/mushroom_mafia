from sqlalchemy import (
    Column, Integer, String, DateTime, Enum, ForeignKey, UniqueConstraint, Index,
)
...
class RelayConfig(Base):
    ...
    threshold_param = Column(Enum(ThresholdParameter), nullable=True)  # which sensor drives this relay in AUTO
    action_on_high = Column(String(3), default="ON")   # what to do when value > max (ON or OFF)
    action_on_low = Column(String(3), default="OFF")   # what to do when value < min (ON or OFF)
